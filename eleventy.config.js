...
// Shortcode for ASIN cards START
	const asinProducts = JSON.parse(
		fs.readFileSync("./_data/asinProducts.json", "utf-8")
	);

	eleventyConfig.addNunjucksAsyncShortcode("asin", async function(asin) {
  const p = asinProducts[asin];
  if (!p) {
    return `<div class="post-card missing">Product ${asin} not found in cache.</div>`;
  }
  return `
   <a href="${p.url}" class="asin-card" target="_blank" rel="nofollow noopener">
  <img src="${p.image}" alt="${p.title}">
  <h3>${p.title}</h3>
  <div class="asin-card-footer">
    <span class="button">View on Amazon</span>
    <p><em>*affiliate link</em></p>
  </div>
</a>
  `;
});
// Shortcode for ASIN cards END
